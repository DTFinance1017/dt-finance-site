      ...
      >
        <div className="max-w-[1440px] mx-auto px-6 sm:px-10 lg:px-16">
          <div className="flex items-center justify-between h-16 lg:h-[72px]">
                  ...
                  <button
                    ...
                    className={`px-4 py-2 text-sm font-medium rounded-lg transition-colors duration-200 ${
                      active
                        ...
                        : "text-[#1B4158]/75 hover:text-[#1B4158]/90 hover:bg-[#1B4158]/4"
                    }`}
                  >
                    {link.label}
                  </button>
            ...
            <button className="md:hidden text-[#1B4158]/78 hover:text-[#1B4158] p-2 rounded-lg transition-colors" onClick={() => setMobileOpen(!mobileOpen)}>
          ...
          <div className="md:hidden bg-white/97 backdrop-blur-md border-t border-[#1B4158]/8">
            <div className="max-w-[1440px] mx-auto px-6 py-4 space-y-1">
              {links.map((link) => {
                const active = location === link.path;
                return (
                  <button key={link.label} onClick={() => navTo(link.path)} className={`block w-full text-left py-2.5 px-3 text-sm font-medium transition-colors rounded-lg hover:bg-[#1B4158]/4 ${active ? "text-[#B5891A]" : "text-[#1B4158]/78 hover:text-[#1B4158]/90"}`}>
                    {link.label}
                  </button>
                );
              })}
